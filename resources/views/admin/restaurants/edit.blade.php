@extends('layouts.vertical', ['title' => 'Edit Restaurant', 'sub_title' => 'Admin', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
                            ...
                            <form action="{{ route('admin.restaurants.update', $restaurant) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" id="restaurant-name" name="name" value="{{ old('name', $restaurant->name) }}" required
                                    ...
                                    <div>
                                        ...
                                        <input type="text" id="restaurant-phone" name="phone" value="{{ old('phone', $restaurant->phone) }}" required
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <input type="text" id="restaurant-latitude" name="latitude" value="{{ old('latitude', $restaurant->latitude) }}"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label for="restaurant-longitude" class="block text-sm font-medium text-gray-700">Longitude</label>
                                            <input type="text" id="restaurant-longitude" name="longitude" value="{{ old('longitude', $restaurant->longitude) }}"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select id="restaurant-type" name="type"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                            <option value="restaurant" {{ old('type', $restaurant->type) === 'restaurant' ? 'selected' : '' }}>Restaurant</option>
                                            <option value="shop_center" {{ old('type', $restaurant->type) === 'shop_center' ? 'selected' : '' }}>Shop Center</option>
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        @if ($restaurant->getFirstMediaUrl('image'))
                                            <div class="mt-2">
                                                <img src="{{ $restaurant->getFirstMediaUrl('image') }}" alt="Restaurant Image" class="h-48 w-auto object-cover rounded-lg">
                                            </div>
                                        @endif
                                    </div>

                                    @if ($restaurant->admin)
                                        <h3 class="text-lg font-medium pt-4">Restaurant Admin Information</h3>

                                        <div>
                                            <label for="admin-name" class="block text-sm font-medium text-gray-700">Admin Name</label>
                                            <input type="text" id="admin-name" name="admin_name" value="{{ old('admin_name', $restaurant->admin->name) }}"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>

                                        <div>
                                            <label for="admin-email" class="block text-sm font-medium text-gray-700">Admin Email</label>
                                            <input type="email" id="admin-email" name="admin_email" value="{{ old('admin_email', $restaurant->admin->email) }}"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>

                                        <div>
                                            <label for="admin-phone" class="block text-sm font-medium text-gray-700">Admin Phone</label>
                                            <input type="text" id="admin-phone" name="admin_phone" value="{{ old('admin_phone', $restaurant->admin->phone) }}"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>

                                        <div>
                                            <label for="admin-password" class="block text-sm font-medium text-gray-700">Admin Password (leave blank to keep current)</label>
